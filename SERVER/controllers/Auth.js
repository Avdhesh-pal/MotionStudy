...
export const signup = async (req, res) => {
  try {
    ...
    console.log("Signup request email:", email, "normalized:", normalizedEmail)
    // Find the most recent OTP for the email
    const recentOtp = await OTP.findOne({ email: normalizedEmail }).sort({ createdAt: -1 })
    console.log("Found OTP for signup check:", recentOtp)
    if (!recentOtp) {
    ...
    // Normalize types before comparing (client may send number or string)
    ...
    // Create the user
    // Determine approval based on account type
